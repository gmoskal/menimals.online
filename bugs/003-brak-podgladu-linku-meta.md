...
- **AC-1:** zielony lokalnie — realny serwer zwraca komplet tagów crawlerowi;
  końcowa weryfikacja cache Meta czeka na deploy.
- **AC-2:** zielony — test integracyjny potwierdza robots, oba obrazy i headers.
- **AC-3:** zielony — wszystkie trzy pliki mają identyczny SHA-256; końcowy
  render Meta czeka na deploy.
...
- `~/tmp/codex/bug-003-retro` — detached worktree bazy `85e39bd` służący
  wyłącznie do finalnego retroaktywnego RED; do usunięcia po przebiegu.
- Proces Next uruchamiany przez test — test rejestruje `context.after` i kończy
  go przez `SIGTERM` (z awaryjnym `SIGKILL`); po bramkach zostanie sprawdzone,
  że żaden proces nie pozostał.
- `bugs/assets/003-brak-podgladu-linku-meta/` — ignorowany, zachowany dowód
  bieżącego otwartego raportu zgodnie z retencją skilla.
- Karta Sharing Debugger Meta w Chrome — do zamknięcia po końcowej weryfikacji.
